<?php
class Faq_Categories_Model extends MY_Model
{
    ...
    public function get_faq_categories($options = array())
    {
        if(SLUG_ACTIVE==0){
            ...
            $this->db->select('faq_categories.*,
                        ...
                        slug.id slug_id,
                    ');
            $this->db->join('slug', 'slug.type_id = faq_categories.id AND ' . $this->db->dbprefix . 'slug.type ='.SLUG_TYPE_FAQ_CATEGORIES, 'left');
        }
        
        if (isset($options['id']))
            $this->db->where('faq_categories.id', $options['id']);
        if(isset($options['notid']))
            $this->db->where('faq_categories.id <> ', $options['notid']);
        // lọc ngôn ngữ
        if(isset($options['lang']))
            $this->db->where('faq_categories.lang', $options['lang']);
        
        if (isset($options['parent_id']) && $options['parent_id'] != DEFAULT_COMBO_VALUE)
            $this->db->where('faq_categories.parent_id', $options['parent_id']);
        if (isset($options['keyword']))
            $this->db->like('faq_categories.category', $options['keyword']);
        // Loc theo trang
        ...
        $this->db->order_by('faq_categories.parent_id, faq_categories.position');
    ...
    public function get_faq_categories_count($options = array())
    {
        return count($this->get_faq_categories($options));
    }

    function get_faq_categories_array_by_parent_id($options = array())
    {
        ...
        $categories = $this->get_faq_categories(array('lang' => $options['lang'],'notid' => $options['notid']));
...
//        print_r($ordered_categories);die;
    ...
    public function get_faq_categories_combo($options = array())
    {
        ...
        $cats = $this->get_faq_categories_array_by_parent_id($options);
        if(isset($cats)){
    ...
    function get_faq_categories_object_array_by_parent_id($options = array())
    {
        // Nếu không truyền vào tham số của danh mục cha, thì trả về null
        if (!isset($options['parent_id'])) return null;
        if (!isset($options['lang'])) $options['lang'] = DEFAULT_LANGUAGE;
        ...
        // Lấy ra tất cả các danh mục
        $categories = $this->get_faq_categories(array('lang' => $options['lang']));
        $ordered_categories = array();
    ...
    public function get_level_faq_category($options=array('parent_id'=>0))
    {
    ...
    public function get_list_cat($options=array()){
        $categories = array();
        $cats = $this->get_faq_categories($options);
        if($cats){
            foreach($cats as $key => $cat){
                $id = $cat->id;
                $count = $this->faq_model->get_faq_count(array('status'=>STATUS_ACTIVE,'cat_id'=>$id));
                if($count > 0){
                    $categories2[] = array(
                        'id' => $id,
                        'title' => $cat->category,
            ...
            if(isset($options['isall']) && $options['isall']==TRUE){
                $cat = $this->get_faq_categories(array('id'=>$options['parent_id']));
                $categories1[] = array(
            ...
            $cats2 = $this->get_faq_categories(array('id'=>$options['parent_id']));
    ...
    public function get_cat_ids_array($parent_id){
        ...
        $list_ids = $this->get_faq_categories(array('parent_id'=>$parent_id));
        if(isset($list_ids)){
    ...
    public function item_to_sort_faq_cat($options=array())
    {
        if (!isset($options['id'])) {
            return NULL;
        }
        // Lay ban ghi can up len //, 'lang'=>$options['lang']
        $this_cat = $this->get_faq_categories(array('id'=>$options['id']));
    ...
    public function position_to_add_faq_cat($options=array())
    {
        if (!isset($options['parent_id'])) {
            return NULL;
        }
        ...
        $faq_cat = $this->db
                ...
                ->limit(1)
                ->get('faq_categories');
        if($faq_cat->num_rows() > 0){
            $faq_cat = $faq_cat->row(0);
            $position_add = $faq_cat->position + 1;
        }else{
            $position_add = 1;
        }
        return $position_add;
    ...
    public function position_to_edit_faq_cat($options=array())
    {
...
//                ->limit(1)
        ...
        if($faq_cat->num_rows() > 0){
            $faq_cat = $faq_cat->result();
            $position_max = $faq_cat[0]->position;
            foreach($faq_cat as $key => $m){
                if($m->id === $options['id']){
