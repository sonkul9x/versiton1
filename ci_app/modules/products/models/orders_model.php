...
//        $this->output->enable_profiler(TRUE);
    ...
    public function get_orders_date($options = array())
    {
        $this->db->select(' orders.str_created_order,');
        $this->db->select_sum('total');
        $this->db->where('order_status', PAID_ORDER);
        $this->db->group_by("str_created_order"); 
        $query = $this->db->get('orders');
        if($query->num_rows() > 0){
