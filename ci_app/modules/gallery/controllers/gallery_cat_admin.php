<?php
class Gallery_Cat_Admin extends MY_Controller
{
   function  __construct()
    ...
    function browse($para1=DEFAULT_LANGUAGE)
    {
        ...
        $this->phpsession->save('gallery_cat_lang', $options['lang']);
        
        $options['categories'] = $this->gallery_categories_model->get_gallery_categories_object(array('parent_id' => ROOT_CATEGORY_ID,'lang'=>$options['lang']));
        ...
        $this->_view_data['title'] = 'Phân loại bộ sưu tập ảnh' . DEFAULT_TITLE_SUFFIX;
    ...
    function add()
    {
        ...
        // Chuan bi cac the META
        $this->_view_data['title'] = 'Thêm phân loại bộ sưu tập ảnh' . DEFAULT_TITLE_SUFFIX;
    ...
    private function _do_add()
    {
        ...
        if ($this->form_validation->run())
        {
            ...
            $position_add = $this->gallery_categories_model->position_to_add_gallery_cat(array('lang'=>$post_data['lang'],'parent_id'=>$post_data['parent_id']));
            $post_data['position'] = $position_add;
            $insert_id = $this->gallery_categories_model->insert($post_data);
            if(SLUG_ACTIVE>0){
                if(isset($insert_id)){
                    $this->slug_model->insert_slug(array('slug'=>  my_trim($this->input->post('slug'), TRUE),'type'=>SLUG_TYPE_GALLERY_CATEGORIES,'type_id'=>$insert_id));
                }
            }
            redirect(GALLERY_CAT_ADMIN_BASE_URL . '/' . $post_data['lang']);
        }
        return FALSE;
    }
    ...
    private function _get_posted_data()
    {
        ...
        $parent_level = $this->gallery_categories_model->get_level_gallery_category(array('parent_id'=>$parent_id));
    ...
    private function _get_add_form_data()
    {
        ...
        if($this->is_postback())
        {
            ...
            $view_data['categories_combobox']   = $this->gallery_categories_model->
                                                    get_gallery_categories_combo(array('categories_combobox' => 
        ...
        {
            $view_data['lang_combobox'] = $this->utility_model->get_lang_combo(array('lang' => $this->phpsession->get('gallery_cat_lang'), 'extra' => 'onchange="javascript:get_categories_by_lang();" class="btn"'));
            $view_data['categories_combobox']   = $this->gallery_categories_model->
                                                    get_gallery_categories_combo(array('categories_combobox' => 
                                                                                $this->input->post('categories_combobox')
                                                                                , 'is_add_edit_cat' => TRUE
                                                                                , 'lang' => $this->phpsession->get('gallery_cat_lang')
                                                                                , 'extra' => 'class="btn"'
                                                                                ));
        }
        
        $view_data['header']        = 'Thêm phân loại bộ sưu tập ảnh';
        $view_data['button_name']   = 'Lưu dữ liệu';
        $view_data['submit_uri']    = GALLERY_CAT_ADMIN_ADD_URL;
        return $view_data;
    }
    ...
    function edit()
    {
        ...
        $options['header']        = 'Sửa phân loại bộ sưu tập ảnh';
        $options['button_name']   = 'Lưu dữ liệu';
        $options['submit_uri']    = GALLERY_CAT_ADMIN_EDIT_URL;

        // Chuan bi du lieu chinh de hien thi
        $this->_view_data['main_content'] = $this->load->view('cat/cat_form', $options, TRUE);
        // Chuan bi cac the META
        $this->_view_data['title'] = 'Sửa phân loại bộ sưu tập ảnh' . DEFAULT_TITLE_SUFFIX;
    ...
    private function _do_edit()
    {
        ...
        if($this->form_validation->run())
        {
            ...
            $position_edit = $this->gallery_categories_model->position_to_edit_gallery_cat(array(
                'id'=>$data['id'],
                'lang'=>$data['lang'],
                'parent_id'=>$data['parent_id'],
            ));
            $data['position'] = $position_edit;
            $this->gallery_categories_model->update($data);
            if(SLUG_ACTIVE>0){
                $this->slug_model->update_slug(array('slug'=>  my_trim($this->input->post('slug'), TRUE),'type'=>SLUG_TYPE_GALLERY_CATEGORIES,'type_id'=>$data['id']));
            }
            redirect(GALLERY_CAT_ADMIN_BASE_URL . '/' . $data['lang']);
        }
        return FALSE;
    }
    ...
    private function _get_edit_cat_form_data()
    {
        ...
        {
            $categories = $this->gallery_categories_model->get_gallery_categories(array('id' => $id));
        ...
        $options['categories_combobox'] = $this->gallery_categories_model->get_gallery_categories_combo(array('categories_combobox' => $parent_id, 'lang' => $lang, 'is_add_edit_cat' => TRUE, 'extra' => 'class="btn"', 'notid' => $id));
    ...
    function delete()
    {
        ...
        if($this->is_postback()){
            $id = $this->input->post('id');
            $check_gallery = $this->gallery_model->get_gallery_count(array('cat_id'=>$id));
            $categories = $this->gallery_categories_model->get_gallery_categories_count(array('parent_id' => $id));
            if($check_gallery > 0 || $categories > 0){
                $options['error'] = 'Không thể xóa phân loại này vì vẫn tồn tại bộ sưu tập ảnh hoặc phân loại con';
            }
            else{
                if(SLUG_ACTIVE>0){
                    $check_slug = $this->slug_model->get_slug(array('type_id'=>$id,'type'=>SLUG_TYPE_GALLERY_CATEGORIES,'onehit'=>TRUE));
                    if(!empty($check_slug)){
                        $this->slug_model->delete($check_slug->id);
                    }
                }
                $this->gallery_categories_model->delete($id);
                $options['warning'] = 'Đã xóa thành công';
            }
        }
        $lang = $this->phpsession->get('gallery_cat_lang');
        redirect(GALLERY_CAT_ADMIN_BASE_URL . '/' . $lang);
    }
    
    function get_gallery_categories_by_lang()
    {
        $lang = $this->input->post('lang', TRUE);
        $id = $this->input->post('id', TRUE);
        if (!$this->input->post('is_add_edit')) {
            echo $this->gallery_categories_model->get_gallery_categories_combo(array('lang' => $lang, 'notid' => $id, 'extra' => 'class="btn"'));
        } else {
            echo $this->gallery_categories_model->get_gallery_categories_combo(array('lang' => $lang, 'notid' => $id, 'is_add_edit_cat' => TRUE, 'extra' => 'class="btn"'));
        }
    }
    ...
    function up()
    {
        $id = $this->input->post('id');
        $lang = $this->phpsession->get('gallery_cat_lang');
        $this->gallery_categories_model->item_to_sort_gallery_cat(array('id' => $id));
        redirect(GALLERY_CAT_ADMIN_BASE_URL . '/' . $lang);
    }
