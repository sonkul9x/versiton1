...
class Customers extends MY_Controller {
     ...
     * @param type $para2
     */
    public function customers_search($para1 = NULL, $para2 = DEFAULT_LANGUAGE) {
    ...
    public function customers_tags($para1 = NULL, $para2 = DEFAULT_LANGUAGE, $para3 = NULL) {
...
//                    $active_menu = '/dich-vu';
    ...
    public function customers_sign_up($para1 = DEFAULT_LANGUAGE) {
        ...
        $view_data['pass'] = 'Mật khẩu <span style="color: red;">(*)</span>';
        $view_data['re_pass'] = 'Nhập lại mật khẩu <span style="color: red;">(*)</span>';
       // $view_data['info_help'] = '<p style="color: red;">Nếu không đổi mật khẩu vui lòng để trống.</p>';
...
//        }
    ...
    public function set_login($options) {
        ...
        $this->phpsession->save('is_logged_in_cus', TRUE);
        $this->phpsession->save('user_id_cus', $user->id);
        $this->phpsession->save('fullname_cus', $user->fullname);
        $this->phpsession->save('email_cus', $user->email); //địa chỉ email
    ...
    function is_logged_in_cus() {
        if ($this->phpsession->get('is_logged_in_cus'))
            return TRUE;
        else {
    ...
    function logout() {
        ...
        $this->phpsession->save('is_logged_in_cus', false);
        $this->phpsession->clear('user_id_cus');
        $this->phpsession->clear('fullname_cus');
        $this->cart->destroy();
        $this->phpsession->save('roles_id', ROLE_GUESS);
...
//        }
    ...
    public function get_infomation_customers($options = array()) {
        if ($this->is_logged_in_cus()):
            $user = $this->customers_model->get_customers(array('user_id' => $this->phpsession->get('user_id_cus'), 'last_row' => TRUE));
            ...
            $view_data['pass'] = 'Đổi mật khẩu <span style="color: red;">(*)</span>';
            $view_data['re_pass'] = 'Nhập lại mật khẩu mới<span style="color: red;">(*)</span>';
            $view_data['info_help'] = '<p style="color: red;">Nếu không đổi mật khẩu vui lòng để trống.</p>';
    ...
    private function process_update_info() {
        ...
        if ($password != '' && $password != null):
            $this->form_validation->set_rules('password', 'Mật khẩu', 'trim|required|xss_clean|min_length[3]');
            $this->form_validation->set_rules('re_password', 'Xác nhận mật khẩu', 'trim|required|xss_clean|min_length[5]|matches[password]');
        endif;
        $this->form_validation->set_rules('security_code', 'Mã an toàn', 'trim|required|xss_clean|matches_value[' . $this->phpsession->get('captcha') . ']');
        if ($this->form_validation->run()) {
            ...
            $options['id'] = $this->phpsession->get('user_id_cus');

            $this->customers_model->update($options);
            $this->phpsession->save('ok_customers_message', 'ok');
            $user = $this->customers_model->get_customers(array('last_row' => TRUE, 'user_id' => $this->phpsession->get('user_id_cus')));
    ...
    function manager_order($para1 = NULL) {
        ...
        $this->_keywords = 'Đơn hàng' . DEFAULT_KEYWORDS;
    ...
    function detail_order_ajax() {
        ...
        if ($this->is_postback()) {
            $orders = $this->orders_model->get_orders(array('user_id' => $user_id, 'id' => $id,'onehit' =>true));
            ...
            $this->_keywords = 'Chi tiết Đơn hàng' . DEFAULT_KEYWORDS;
