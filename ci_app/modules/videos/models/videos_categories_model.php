...
class Videos_Categories_Model extends MY_Model
{
    ...
    public function get_videos_categories($options = array())
    {
        if(SLUG_ACTIVE==0){
            $this->db->select('videos_categories.*');
        }else{
            $this->db->select('videos_categories.*,
                        slug.slug,
                        slug.id slug_id,
                    ');
            $this->db->join('slug', 'slug.type_id = videos_categories.id AND ' . $this->db->dbprefix . 'slug.type ='.SLUG_TYPE_VIDEOS_CATEGORIES, 'left');
        }
        
        if (isset($options['id']))
            $this->db->where('videos_categories.id', $options['id']);
        
        if(isset($options['current_id']))
            $this->db->where('videos_categories.id != ',$options['current_id']);
        
        if(isset($options['notid']))
            $this->db->where('videos_categories.id != ',$options['notid']);

        if (isset($options['status']))
            $this->db->where('videos_categories.status', $options['status']);

        if (isset($options['lang']))
            $this->db->where('videos_categories.lang', $options['lang']);

        if (isset($options['parent_id']) && $options['parent_id'] != DEFAULT_COMBO_VALUE)
            $this->db->where('videos_categories.parent_id', $options['parent_id']);
        
        if (isset($options['keyword']))
            $this->db->like('videos_categories.category', $options['keyword']);
        ...
        $this->db->order_by('videos_categories.parent_id, videos_categories.position');

        $query = $this->db->get('videos_categories');
    ...
    public function get_videos_categories_count($options = array()) {
        return count($this->get_videos_categories($options));
    }

    function get_videos_categories_array_by_parent_id($options = array()) {
        ...
        $categories = $this->get_videos_categories(array('lang' => $options['lang'],'notid'=>$options['notid']));
...
//        print_r($ordered_categories);die;
    ...
    public function get_videos_categories_combo($options = array()) {
        ...
        $cats = $this->get_videos_categories_array_by_parent_id($options);
    ...
    function delete_videos_category($id = 0) {
//        $this->db->delete('videos', array('cat_id' => $id));
        $this->db->delete('videos_categories', array('id' => $id));
        $this->db->delete('videos_categories', array('parent_id' => $id));
    }
    
    public function get_level_videos_category($options=array('parent_id'=>0))
    {
    ...
    function get_videos_categories_object($options = array()) {
        // Nếu không truyền vào tham số của danh mục cha, thì trả về null
        if (!isset($options['parent_id']))
            return null;
        if (!isset($options['lang']))
        ...
        // Lấy ra tất cả các danh mục
        $categories = $this->get_videos_categories(array('lang'=>$options['lang']));
        $ordered_categories = array();
...
//        print_r($ordered_categories);die;
    ...
    public function item_to_sort_videos_cat($options=array())
    {
        if (!isset($options['id'])) {
            return NULL;
        }
        // Lay ban ghi can up len //, 'lang'=>$options['lang']
        $this_cat = $this->get_videos_categories(array('id'=>$options['id']));
    ...
    public function position_to_add_videos_cat($options=array())
    {
        if (!isset($options['parent_id'])) {
            return NULL;
        }
        ...
        $videos_cat = $this->db
                ...
                ->limit(1)
                ->get('videos_categories');
        if($videos_cat->num_rows() > 0){
            $videos_cat = $videos_cat->row(0);
            $position_add = $videos_cat->position + 1;
        }else{
            $position_add = 1;
        }
        return $position_add;
    ...
    public function position_to_edit_videos_cat($options=array())
    {
...
//                ->limit(1)
        ...
        if($videos_cat->num_rows() > 0){
            $videos_cat = $videos_cat->result();
            $position_max = $videos_cat[0]->position;
            foreach($videos_cat as $key => $m){
                if($m->id === $options['id']){
