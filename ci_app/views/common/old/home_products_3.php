<?php $results = modules::run('products/get_products_home',array('top_seller'=>TRUE)); ?>
<?php if(!empty($results)){ ?>
<div class="homebox3">
    <div class="title">
        <a href="/san-pham-ban-chay" class="heading">Sản Phẩm Bán Chạy Nhất</a>
        <a href="/san-pham-ban-chay" class="view-all">Xem tất cả <i class="fa fa-arrow-right"></i></a>
    </div>
    <div class="clearfix"></div>
    <div role="tabpanel" class="content">
        <ul class="nav nav-tabs" role="tablist">
            <?php foreach($results as $k => $v){ ?>
            <li role="presentation" <?php if($k==0){ ?>class="active"<?php } ?>><a href="#sale3_<?php echo $v['category_slug']; ?>" aria-controls="sale3_<?php echo $v['category_slug']; ?>" role="tab" data-toggle="tab"><?php echo $v['category']; ?></a></li>
            <?php } ?>
        </ul>
        <div class="tab-content">
            <?php foreach($results as $k => $v){ ?>
            <div role="tabpanel" class="tab-pane sale3_<?php echo $v['category_slug']; ?> <?php if($k==0){ ?>active<?php } ?>" id="sale3_<?php echo $v['category_slug']; ?>">
                <div class="row">
                <?php $data = $v['products'];
                    if(!empty($data)){
