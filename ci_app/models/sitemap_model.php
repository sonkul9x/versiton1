...
EOB;
    ...
    private function _generate_latest_news($number_of_post=10000)
    {
        $today  = date('Y-m-d');
        $output = '';
        $query = $this->db->select('news.id, news.title, news.cat_id, news.summary, news.lang, news_categories.category, slug.slug slug, slug.id slug_id')
                            ->join('news_categories', 'news.cat_id = news_categories.id', 'left')
                            ->join('slug', 'slug.type_id = news.id AND ' . $this->db->dbprefix . 'slug.type = '.SLUG_TYPE_NEWS, 'left')
...
EOB;
    ...
    private function _generate_categories_subs_news()
    {
        $today  = date('Y-m-d');
        $output = '';
        $query = $this->db->select('news_categories.*, slug.slug, slug.id slug_id')
                            ->join('slug', 'slug.type_id = news_categories.id AND ' . $this->db->dbprefix . 'slug.type = '.SLUG_TYPE_NEWS_CATEGORIES, 'left')
                            ->get('news_categories');
        $categories = $query->result();
        foreach($categories as $category)
        {
//            $loc = get_base_url($category->lang). url_title($category->category, 'dash', TRUE) . '-n' . $category->id;
            $loc = get_base_url() . $category->slug;
            $output .= <<<EOB
<url>
    <loc>{$loc}</loc>
    <lastmod>{$today}</lastmod>
...
EOB;
    ...
    private function _generate_latest_faq($number_of_post=10000)
    {
        ...
        $output = '';
        $query =
        $this->db->select('faq.id, faq.title, faq.cat_id, faq.summary, faq.lang, faq_categories.category, slug.slug, slug.id slug_id')
                            ->join('faq_categories', 'faq.cat_id = faq_categories.id', 'left')
                            ->join('slug', 'slug.type_id = faq.id AND '. $this->db->dbprefix . 'slug.type ='.SLUG_TYPE_FAQ, 'left')
...
EOB;
    ...
    private function _generate_categories_subs_faq()
    {
        $today  = date('Y-m-d');
        $output = '';
        $query = $this->db->select('faq_categories.*, slug.slug, slug.id slug_id')
                            ->join('slug', 'slug.type_id = faq_categories.id AND ' . $this->db->dbprefix . 'slug.type ='.SLUG_TYPE_FAQ_CATEGORIES, 'left')
                            ->get('faq_categories');
        $categories = $query->result();
        foreach($categories as $category)
        {
...
EOB;
    ...
    private function _generate_latest_posts($number_of_post=10000)
    {
        ...
        $query =
        $this->db->select('products.id, products.product_name, products.categories_id, products_categories.category, product_images.position, product_images.image_name, products.lang, slug.slug, slug.id slug_id')
                            ...
                            ->join('slug', 'slug.type_id = products.id AND ' . $this->db->dbprefix . 'slug.type ='.SLUG_TYPE_PRODUCTS, 'left')
...
EOB;
    ...
    private function _generate_categories_subs()
    {
        $today  = date('Y-m-d');
        $output = '';
        $query = $this->db->select('products_categories.*, slug.slug, slug.id slug_id')
                ->join('slug', 'slug.type_id = products_categories.id AND ' . $this->db->dbprefix . 'slug.type ='.SLUG_TYPE_PRODUCTS_CATEGORIES, 'left')
